...
### 1. 安装数据包
```bash
# 将 cobblemon_lucky_food_datapack 文件夹复制到:
...
### 3. 获得幸运食物
```
/function cobblemon_lucky_food:give_lucky_food
```
...
### 修改宝可梦等级
编辑 `data/cobblemon_lucky_food/function/spawn_normal.mcfunction`:
```mcfunction
# 将 level=50 改为其他等级
...
### 修改闪光几率
编辑 `data/cobblemon_lucky_food/function/on_eat.mcfunction`:
```mcfunction
...
A: 幸运食物有:
   - 金色名称 "✨ 幸运食物 ✨"
   - 描述性 Lore
   - 只能通过 /function cobblemon_lucky_food:give_lucky_food 获得
   普通食物不会触发任何效果
```
...
```
1. 安装数据包
2. /function cobblemon_lucky_food:give_lucky_food
3. 手持幸运食物（金色名称）
4. 右键吃完
5. 享受惊喜！✨
```
